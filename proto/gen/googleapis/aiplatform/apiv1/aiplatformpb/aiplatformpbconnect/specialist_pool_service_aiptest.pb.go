...
func (fx SpecialistPoolServiceTestSuite) TestSpecialistPool(ctx context.Context, options SpecialistPoolServiceSpecialistPoolTestSuiteConfig) {
	fx.T.Run("SpecialistPool", func(t *testing.T) {
		...
		options.Service = func() SpecialistPoolServiceClient { return fx.Client }
		options.test(t)
	})
}
...
type SpecialistPoolServiceSpecialistPoolTestSuiteConfig struct {
	...
	Service func() SpecialistPoolServiceClient
	...
	Create func(parent string) *aiplatformpb.SpecialistPool
	// Update should return a resource which is valid to update, i.e.
	// all required fields set.
	Update func(parent string) *aiplatformpb.SpecialistPool
...
func (fx *SpecialistPoolServiceSpecialistPoolTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().CreateSpecialistPool(fx.Context(), connect.NewRequest(&aiplatformpb.CreateSpecialistPoolRequest{
			Parent:         "",
...
func (fx *SpecialistPoolServiceSpecialistPoolTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().GetSpecialistPool(fx.Context(), connect.NewRequest(&aiplatformpb.GetSpecialistPoolRequest{
			Name: "",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...
func (fx *SpecialistPoolServiceSpecialistPoolTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		msg.Name = ""
		_, err := fx.Service().UpdateSpecialistPool(fx.Context(), connect.NewRequest(&aiplatformpb.UpdateSpecialistPoolRequest{
			SpecialistPool: msg,
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...

	{
		...
		t.Run("required fields", func(t *testing.T) {
			...
			t.Run(".name", func(t *testing.T) {
				fx.maybeSkip(t)
				msg := proto.Clone(created).(*aiplatformpb.SpecialistPool)
				container := msg
				if container == nil {
					t.Skip("not reachable")
				}
...
func (fx *SpecialistPoolServiceSpecialistPoolTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().ListSpecialistPools(fx.Context(), connect.NewRequest(&aiplatformpb.ListSpecialistPoolsRequest{
			Parent: "invalid resource name",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...

	{
		...
		parentMsgs := make([]*aiplatformpb.SpecialistPool, resourcesCount)
		...
		t.Run("isolation", func(t *testing.T) {
			fx.maybeSkip(t)
			listResp, err := fx.Service().ListSpecialistPools(fx.Context(), connect.NewRequest(&aiplatformpb.ListSpecialistPoolsRequest{
				Parent:   parent,
				PageSize: 999,
			}))
			assert.NilError(t, err)
			...
			assert.DeepEqual(
				...
				cmpopts.SortSlices(func(a, b *aiplatformpb.SpecialistPool) bool {
		...
		t.Run("one by one", func(t *testing.T) {
			fx.maybeSkip(t)
			msgs := make([]*aiplatformpb.SpecialistPool, 0, resourcesCount)
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				_, err := fx.Service().DeleteSpecialistPool(fx.Context(), connect.NewRequest(&aiplatformpb.DeleteSpecialistPoolRequest{
...
func (fx *SpecialistPoolServiceSpecialistPoolTestSuiteConfig) create(t *testing.T, parent string) *aiplatformpb.SpecialistPool {
	t.Helper()
