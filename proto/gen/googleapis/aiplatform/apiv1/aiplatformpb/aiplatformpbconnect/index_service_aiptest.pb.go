...
func (fx IndexServiceTestSuite) TestIndex(ctx context.Context, options IndexServiceIndexTestSuiteConfig) {
	fx.T.Run("Index", func(t *testing.T) {
		...
		options.Service = func() IndexServiceClient { return fx.Client }
		options.test(t)
	})
}
...
type IndexServiceIndexTestSuiteConfig struct {
	...
	Service func() IndexServiceClient
	...
	Create func(parent string) *aiplatformpb.Index
	// Update should return a resource which is valid to update, i.e.
	// all required fields set.
	Update func(parent string) *aiplatformpb.Index
...
func (fx *IndexServiceIndexTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().CreateIndex(fx.Context(), connect.NewRequest(&aiplatformpb.CreateIndexRequest{
			Parent: "",
...
func (fx *IndexServiceIndexTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().GetIndex(fx.Context(), connect.NewRequest(&aiplatformpb.GetIndexRequest{
			Name: "",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
	...
	t.Run("exists", func(t *testing.T) {
		...
		getResp, err := fx.Service().GetIndex(fx.Context(), connect.NewRequest(&aiplatformpb.GetIndexRequest{
			Name: created.Name,
		}))
		msg := getResp.Msg
...
func (fx *IndexServiceIndexTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		msg.Name = ""
		_, err := fx.Service().UpdateIndex(fx.Context(), connect.NewRequest(&aiplatformpb.UpdateIndexRequest{
			Index: msg,
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
	...
	t.Run("etag updated", func(t *testing.T) {
		...
		msg := fx.Update(parent)
		msg.Name = created.Name
		updateResp, err := fx.Service().UpdateIndex(fx.Context(), connect.NewRequest(&aiplatformpb.UpdateIndexRequest{
			Index: msg,
		}))
...

	{
		...
		t.Run("required fields", func(t *testing.T) {
			...
			t.Run(".display_name", func(t *testing.T) {
				fx.maybeSkip(t)
				msg := proto.Clone(created).(*aiplatformpb.Index)
				container := msg
				if container == nil {
					t.Skip("not reachable")
				}
...
func (fx *IndexServiceIndexTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().ListIndexes(fx.Context(), connect.NewRequest(&aiplatformpb.ListIndexesRequest{
			Parent: "invalid resource name",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...

	{
		...
		parentMsgs := make([]*aiplatformpb.Index, resourcesCount)
		...
		t.Run("isolation", func(t *testing.T) {
			fx.maybeSkip(t)
			listResp, err := fx.Service().ListIndexes(fx.Context(), connect.NewRequest(&aiplatformpb.ListIndexesRequest{
				Parent:   parent,
				PageSize: 999,
			}))
			...
			assert.DeepEqual(
				...
				parentMsgs,
				response.Indexes,
				cmpopts.SortSlices(func(a, b *aiplatformpb.Index) bool {
		...
		t.Run("one by one", func(t *testing.T) {
			fx.maybeSkip(t)
			msgs := make([]*aiplatformpb.Index, 0, resourcesCount)
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				_, err := fx.Service().DeleteIndex(fx.Context(), connect.NewRequest(&aiplatformpb.DeleteIndexRequest{
...
func (fx *IndexServiceIndexTestSuiteConfig) testDelete(t *testing.T) {
	...
	t.Run("already deleted", func(t *testing.T) {
		...
		parent := fx.nextParent(t, false)
		created := fx.create(t, parent)
		deleteResp, err := fx.Service().DeleteIndex(fx.Context(), connect.NewRequest(&aiplatformpb.DeleteIndexRequest{
			Name: created.Name,
		}))
...
func (fx *IndexServiceIndexTestSuiteConfig) create(t *testing.T, parent string) *aiplatformpb.Index {
	t.Helper()
