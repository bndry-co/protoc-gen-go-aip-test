...
func (fx ScheduleServiceTestSuite) TestSchedule(ctx context.Context, options ScheduleServiceScheduleTestSuiteConfig) {
	fx.T.Run("Schedule", func(t *testing.T) {
		...
		options.Service = func() ScheduleServiceClient { return fx.Client }
		options.test(t)
	})
}
...
type ScheduleServiceScheduleTestSuiteConfig struct {
	...
	Service func() ScheduleServiceClient
	...
	Create func(parent string) *aiplatformpb.Schedule
	// Update should return a resource which is valid to update, i.e.
	// all required fields set.
	Update func(parent string) *aiplatformpb.Schedule
...
func (fx *ScheduleServiceScheduleTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().CreateSchedule(fx.Context(), connect.NewRequest(&aiplatformpb.CreateScheduleRequest{
			Parent:   "",
	...
	t.Run("create time", func(t *testing.T) {
		...
		createResp, err := fx.Service().CreateSchedule(fx.Context(), connect.NewRequest(&aiplatformpb.CreateScheduleRequest{
			Parent:   parent,
...
func (fx *ScheduleServiceScheduleTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().GetSchedule(fx.Context(), connect.NewRequest(&aiplatformpb.GetScheduleRequest{
			Name: "",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...
func (fx *ScheduleServiceScheduleTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		msg.Name = ""
		_, err := fx.Service().UpdateSchedule(fx.Context(), connect.NewRequest(&aiplatformpb.UpdateScheduleRequest{
			Schedule: msg,
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...

	{
		...
		t.Run("required fields", func(t *testing.T) {
			...
			t.Run(".create_pipeline_job_request.parent", func(t *testing.T) {
				fx.maybeSkip(t)
				msg := proto.Clone(created).(*aiplatformpb.Schedule)
				container := msg.GetCreatePipelineJobRequest()
				if container == nil {
					t.Skip("not reachable")
				}
...
func (fx *ScheduleServiceScheduleTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().ListSchedules(fx.Context(), connect.NewRequest(&aiplatformpb.ListSchedulesRequest{
			Parent: "invalid resource name",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...

	{
		...
		parentMsgs := make([]*aiplatformpb.Schedule, resourcesCount)
		...
		t.Run("isolation", func(t *testing.T) {
			fx.maybeSkip(t)
			listResp, err := fx.Service().ListSchedules(fx.Context(), connect.NewRequest(&aiplatformpb.ListSchedulesRequest{
				Parent:   parent,
				PageSize: 999,
			}))
			assert.NilError(t, err)
			...
			assert.DeepEqual(
				...
				parentMsgs,
				response.Schedules,
				cmpopts.SortSlices(func(a, b *aiplatformpb.Schedule) bool {
		...
		t.Run("one by one", func(t *testing.T) {
			fx.maybeSkip(t)
			msgs := make([]*aiplatformpb.Schedule, 0, resourcesCount)
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				_, err := fx.Service().DeleteSchedule(fx.Context(), connect.NewRequest(&aiplatformpb.DeleteScheduleRequest{
...
func (fx *ScheduleServiceScheduleTestSuiteConfig) create(t *testing.T, parent string) *aiplatformpb.Schedule {
	t.Helper()
	createResp, err := fx.Service().CreateSchedule(fx.Context(), connect.NewRequest(&aiplatformpb.CreateScheduleRequest{
		Parent:   parent,
		Schedule: fx.Create(parent),
