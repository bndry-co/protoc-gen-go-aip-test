...
func (fx IndexEndpointServiceTestSuite) TestIndexEndpoint(ctx context.Context, options IndexEndpointServiceIndexEndpointTestSuiteConfig) {
	fx.T.Run("IndexEndpoint", func(t *testing.T) {
		...
		options.Service = func() IndexEndpointServiceClient { return fx.Client }
		options.test(t)
	})
}
...
type IndexEndpointServiceIndexEndpointTestSuiteConfig struct {
	...
	Service func() IndexEndpointServiceClient
	...
	Create func(parent string) *aiplatformpb.IndexEndpoint
	// Update should return a resource which is valid to update, i.e.
	// all required fields set.
	Update func(parent string) *aiplatformpb.IndexEndpoint
...
func (fx *IndexEndpointServiceIndexEndpointTestSuiteConfig) testCreate(t *testing.T) {
	...
	t.Run("missing parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().CreateIndexEndpoint(fx.Context(), connect.NewRequest(&aiplatformpb.CreateIndexEndpointRequest{
			Parent:        "",
...
func (fx *IndexEndpointServiceIndexEndpointTestSuiteConfig) testGet(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().GetIndexEndpoint(fx.Context(), connect.NewRequest(&aiplatformpb.GetIndexEndpointRequest{
			Name: "",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...
func (fx *IndexEndpointServiceIndexEndpointTestSuiteConfig) testUpdate(t *testing.T) {
	...
	t.Run("missing name", func(t *testing.T) {
		...
		msg.Name = ""
		_, err := fx.Service().UpdateIndexEndpoint(fx.Context(), connect.NewRequest(&aiplatformpb.UpdateIndexEndpointRequest{
			IndexEndpoint: msg,
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...

	{
		...
		t.Run("required fields", func(t *testing.T) {
			...
			t.Run(".display_name", func(t *testing.T) {
				fx.maybeSkip(t)
				msg := proto.Clone(created).(*aiplatformpb.IndexEndpoint)
				container := msg
				if container == nil {
					t.Skip("not reachable")
				}
...
func (fx *IndexEndpointServiceIndexEndpointTestSuiteConfig) testList(t *testing.T) {
	...
	t.Run("invalid parent", func(t *testing.T) {
		fx.maybeSkip(t)
		_, err := fx.Service().ListIndexEndpoints(fx.Context(), connect.NewRequest(&aiplatformpb.ListIndexEndpointsRequest{
			Parent: "invalid resource name",
		}))
		assert.Equal(t, connect.CodeInvalidArgument, connect.CodeOf(err), err)
	})
...

	{
		...
		parentMsgs := make([]*aiplatformpb.IndexEndpoint, resourcesCount)
		...
		t.Run("isolation", func(t *testing.T) {
			fx.maybeSkip(t)
			listResp, err := fx.Service().ListIndexEndpoints(fx.Context(), connect.NewRequest(&aiplatformpb.ListIndexEndpointsRequest{
				Parent:   parent,
				PageSize: 999,
			}))
			...
			assert.DeepEqual(
				...
				cmpopts.SortSlices(func(a, b *aiplatformpb.IndexEndpoint) bool {
		...
		t.Run("one by one", func(t *testing.T) {
			fx.maybeSkip(t)
			msgs := make([]*aiplatformpb.IndexEndpoint, 0, resourcesCount)
		...
		t.Run("deleted", func(t *testing.T) {
			...
			for i := 0; i < deleteCount; i++ {
				_, err := fx.Service().DeleteIndexEndpoint(fx.Context(), connect.NewRequest(&aiplatformpb.DeleteIndexEndpointRequest{
...
func (fx *IndexEndpointServiceIndexEndpointTestSuiteConfig) create(t *testing.T, parent string) *aiplatformpb.IndexEndpoint {
	t.Helper()
