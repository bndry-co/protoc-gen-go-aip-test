...
type serviceGenerator struct {
	...
	// TODO: potentially use a serviceClient field here that specifies the client in another package/directory (instead of assuming a client in the same package)
}
...
func (s *serviceGenerator) generateFixture(f *protogen.GeneratedFile) {
	...
	// This "Client" name matches the service name generated by protoc-gen-connect-go.
	// See: https://github.com/connectrpc/connect-go/blob/a34cd270cd7fee78df46cbd0afc4ab24d5eac2cf/cmd/protoc-gen-connect-go/main.go#L676
	...
	f.P("T *", testingT)
	f.P("// Client to test.")
	f.P("Client  ", serviceClient.GoName)
	f.P()
...
func (s *serviceGenerator) generateTestMethods(f *protogen.GeneratedFile) {
	...
	for _, resource := range s.resources {
		...
		f.P("options.Service = func() ", serviceClient.GoName, " { return fx.Client", "}")
		f.P("options.test(t)")
		f.P("})")
		f.P("}")
		f.P()
